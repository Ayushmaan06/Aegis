...
var chunk_SM4P4XOJ_exports = {};
__export(chunk_SM4P4XOJ_exports, {
...
module.exports = __toCommonJS(chunk_SM4P4XOJ_exports);
...
async function formatSchema({ schemas }, inputFormattingOptions) {
  if (process.env.FORCE_PANIC_PRISMA_SCHEMA) {
    handleFormatPanic(() => {
      import_chunk_SLNABFCV.prismaSchemaWasm.debug_panic();
    });
  }
  ...
  const { formattedMultipleSchemas, lintDiagnostics } = handleFormatPanic(() => {
    ...
    const lintDiagnostics2 = (0, import_chunk_ZVFQ7CYU.lintSchema)({ schemas: formattedMultipleSchemas2 });
    return { formattedMultipleSchemas: formattedMultipleSchemas2, lintDiagnostics: lintDiagnostics2 };
  });
  const lintWarnings = (0, import_chunk_ZVFQ7CYU.getLintWarningsAsText)(lintDiagnostics);
...
function formatWasm(schema, documentFormattingParams) {
  const formattedSchema = import_chunk_SLNABFCV.prismaSchemaWasm.format(schema, JSON.stringify(documentFormattingParams));
...
async function processSchemaResult({
  ...
  const configFromPsl = await (0, import_chunk_II5NF2VA.getConfig)({ datamodel: schemaResult.schemas });
...
async function getGenerators(options) {
  ...
  const dmmf = await (0, import_chunk_RLUV56AY.getDMMF)({
...
You need to define \`output\` in the generator block in the schema file.`
        ...
        const datamodel = (0, import_chunk_ZR36NJZI.mergeSchemas)({ schemas: schemaContext.schemaFiles });
        const options2 = {
          datamodel,
